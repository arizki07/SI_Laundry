...
class RatingList extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = RatingModel::all();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if ($row->status == 'ya') {
                        return '<span class="badge bg-green-lt" style="font-size:11px"><i class="fa-solid fa-check me-2"></i>'.$row->status.'</span>';
                    } elseif ($row->status == 'tidak') {
                        return '<span class="badge bg-danger-lt" style="font-size:11px"><i class="fa-solid fa-x me-2"></i>'.$row->status.'</span>';
                    }
                })
